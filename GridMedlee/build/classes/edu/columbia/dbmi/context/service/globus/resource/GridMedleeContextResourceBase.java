...
import edu.columbia.dbmi.context.common.GridMedleeContextConstants;
import edu.columbia.dbmi.context.stubs.GridMedleeContextResourceProperties;
 ...
 * 
 */
public abstract class GridMedleeContextResourceBase extends ReflectionResource implements Resource
                                                  ,PersistenceCallback
                                                  ,TopicListAccessor
                                                  ,SecureResource
                                                  ,RemoveCallback
	...
	static final Log logger = LogFactory.getLog(GridMedleeContextResourceBase.class);

	private GridMedleeContextResourceConfiguration configuration;
    ...
    public GridMedleeContextResourceBase() {
        try {
            resourcePropertyPersistenceHelper = new gov.nih.nci.cagrid.introduce.servicetools.XmlPersistenceHelper(GridMedleeContextResourceProperties.class,GridMedleeConfiguration.getConfiguration());
            resourcePersistenceHelper = new FilePersistenceHelper(this.getClass(),GridMedleeConfiguration.getConfiguration(),".resource");
        } catch (Exception ex) {
	...
	public void setTerminationTime(Calendar time) {	
		Topic terminationTopic = ((Topic)getResourcePropertySet().get(GridMedleeContextConstants.TERMINATIONTIME));
	...
	public edu.columbia.dbmi.medlee.StatusOptions getStatus(){
		return ((GridMedleeContextResourceProperties) getResourceBean()).getStatus();
	}
	
	public void setStatus(edu.columbia.dbmi.medlee.StatusOptions status ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(GridMedleeContextConstants.STATUS);
		prop.set(0, status);
        //call the first store to persist the resource
        store();
	}
	...
	public GridMedleeContextResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (GridMedleeContextResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
    ...
    public void load(ResourceKey resourceKey) throws ResourceException, NoSuchResourceException, InvalidResourceKeyException {
	  beingLoaded = true;
	   ...
	   GridMedleeContextResourceProperties props = (GridMedleeContextResourceProperties)resourcePropertyPersistenceHelper.load(GridMedleeContextResourceProperties.class, resourceKey.getValue());
       this.initialize(props, GridMedleeContextConstants.RESOURCE_PROPERTY_SET, resourceKey.getValue());
